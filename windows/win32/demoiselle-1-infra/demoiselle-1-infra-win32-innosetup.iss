#define MyAppName "Demoiselle 1 Infra"
#define MyAppVersion "1.0"
#define MyAppPublisher "Comunidade Demoiselle"
#define MyAppDesktopName "Demoiselle Eclipse 3.5"
#define MyAppURL "http://www.frameworkdemoiselle.gov.br"
#define MyAppExeName "\ide\eclipse-3.5\eclipse.exe"
...
;AppVerName={#MyAppVersion} {#MyAppName}
...
OutputDir=.
OutputBaseFilename=demoiselle-1-infra-win32-{#MyAppVersion}
Compression=lzma
...
[Files]
Source: C:\Demoiselle\ide\eclipse-3.5\*; Excludes: configuration\*.log; DestDir: {app}\ide\eclipse-3.5; Flags: ignoreversion recursesubdirs createallsubdirs
...
Source: C:\Demoiselle\server\tomcat-6.0\*; DestDir: {app}\server\tomcat-6.0; Flags: ignoreversion recursesubdirs createallsubdirs
Source: C:\Demoiselle\server\jboss-4.2\*; Excludes: server\default\data,server\default\log,server\default\tmp,server\default\work,bin\*.lck,bin\*.log,bin\*.script; DestDir: {app}\server\jboss-4.2; Flags: ignoreversion recursesubdirs createallsubdirs
Source: C:\Demoiselle\server\jboss-5.1\*; Excludes: server\default\data,server\default\log,server\default\tmp,server\default\work,bin\*.lck,bin\*.log,bin\*.script; DestDir: {app}\server\jboss-5.1; Flags: ignoreversion recursesubdirs createallsubdirs
...
[Registry]
Root: HKLM; Subkey: SYSTEM\CurrentControlSet\Control\Session Manager\Environment; ValueType: string; ValueName: CATALINA_HOME; ValueData: {app}\server\tomcat-6.0
Root: HKLM; Subkey: SYSTEM\CurrentControlSet\Control\Session Manager\Environment; ValueType: string; ValueName: JBOSS_HOME; ValueData: {app}\server\jboss-4.2
...
begin
 ...
 begin
  for I := 0 to GetArrayLength(Versions)-1 do
   ...
   begin
    if ( Versions[I][2]='.' ) and ( ( StrToInt(Versions[I][1]) > 1 ) or ( ( StrToInt(Versions[I][1]) = 1 ) and ( StrToInt(Versions[I][3]) >= 5 ) ) ) then
    begin
     JavaInstalled := true;
 ...
 begin
  Result := true;
    ...
    begin
  Result1 := MsgBox('This tool requires Java Development Kit more than 1.5 to run. Please download and install the JDK and run this setup again. Do you want to download it now?',
